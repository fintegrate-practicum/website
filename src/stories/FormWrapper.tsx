  ...
  return (
    <Box
      ...
      sx={{
        ...
        borderRadius: '1vh',
        boxShadow: 1,
        maxWidth: formWidth === 'short' ? '20vw' : formWidth === 'medium' ? '50vw' : '100vw',
      }}
    >
      {fields.map((field) => (
        <Box key={field.name} sx={{ display: 'flex', flexDirection: 'column', gap: '2vh' }}>
          {field.type === 'text' || field.type === 'email' ? (
            ...
            <>
              <label htmlFor={field.name} style={{ display: 'inline-block', padding: '1vh 2vw', cursor: 'pointer', backgroundColor: '#007bff', color: 'white', borderRadius: '1vh' }}>
                {field.label}
              </label>
              <input
                id={field.name}
      ...
      <Button variant="contained" color="success" type="submit" sx={{ marginTop: '2vh' }}>
        Submit
      </Button>
    </Box>
  );
