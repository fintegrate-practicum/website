...
const ShoppingDetails: React.FC<Props> = ({ amount }) => {
    ...
    const saveDetails = async (data: Record<string, any>) => {
        try {
            ...
            reset()
             // todo: change this to use toast from storybook
            alert("ההזמנה נשמרה בהצלחה");

        } catch (err) {
            console.log(err);
            showErrorToast("שגיאה בשמירת ההזמנה");
        }
    };
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                {productsCart.length > 0 && Carts.map((cart) => (
                    <Card sx={{ width: 200, maxWidth: '100%', boxShadow: 'lg', margin: 2 }}>
                        <Typography>
                            שם המוצר: {cart.product.name}
                        </Typography>
                        <Typography>
                            כמות: {cart.Quantity}
                        </Typography>
                        <Typography>
                            מחיר: {cart.product.packageCost}
                        </Typography>
                    </Card>
                ))}
            </Box>
            <Typography variant="h5">
                סך הכל לתשלום : {amount}
            </Typography>
            <form onSubmit={handleSubmit(saveDetails)}>
                <FormControl >
                    <Typography >  איך תרצו לקבל את המשלוח? </Typography>
                    <RadioGroup
                        row
