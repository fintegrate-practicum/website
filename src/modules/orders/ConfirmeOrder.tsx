...
import {
  getItemById,
  updateItem,
} from '../inventory/Api-Requests/genericRequests';
...
const ConfirmOrder = () => {
  ...
  const updateQuantity = async (id: string, qty: number) => {
    ...
    try {
      ...
      try {
        const response = await getItemById<IComponent>(
          `/inventory/component`,
          id,
        );
        if (response && response.data) {
  ...
  return (
    <Container maxWidth='sm'>
      <Paper
        elevation={3}
        sx={{ padding: '2rem', textAlign: 'center', marginTop: '2rem' }}
      >
        ...
        <Typography
          variant='h5'
          component='h2'
          sx={{ marginTop: '1rem', color: 'primary.main' }}
        >
        ...
        >
          {t(
            'order.A purchase confirmation email will be sent to the email registered in the system',
          )}
        </Typography>
      </Paper>
    </Container>
